...
class AwsDeployFunction {
  ...
  async getIamClient() {
    this.iamClientPromise ||= this.provider
      .getAwsSdkV3Config()
      .then((config) => new IAMClient(config));
    return this.iamClientPromise;
  }
  ...
  async ensureFunctionState() {
    ...
    const callWithRetry = async () => {
      const result = await lambda.send(new GetFunctionCommand(params));
      if (
        result &&
  ...
  async callUpdateFunctionConfiguration(params) {
    ...
    const callWithRetry = async () => {
      try {
        ...
        if (isLambdaResourceConflictError(err)) {
          if (didOneMinutePass) {
            throw new ServerlessError(
  ...
  async updateFunctionConfiguration() {
    ...
    if (functionObj.recursiveLoop) {
      params.RecursiveLoop = functionObj.recursiveLoop;
    }
    ...
    if (!functionObj.layers || !functionObj.layers.some(isObject)) {
    ...
    const remoteDeadLetterTargetArn = remoteFunctionConfiguration.DeadLetterConfig?.TargetArn;

    if (
      functionObj.onError &&
      !isObject(functionObj.onError) &&
      (remoteDeadLetterTargetArn === undefined ? null : remoteDeadLetterTargetArn) !==
        functionObj.onError
    ) {
    ...
    if (functionObj.environment || providerObj.environment) {
      params.Environment.Variables = safeShallowAssign(
        {},
        providerObj.environment,
        functionObj.environment
      );
    }
    if (Object.values(params.Environment.Variables).some((value) => isObject(value))) {
      delete params.Environment;
    } else {
      Object.keys(params.Environment.Variables).forEach((key) => {
        // Matches shell environment variable naming rules.
    ...
    if (functionObj.vpc || providerObj.vpc) {
      ...
      if (Array.isArray(vpc.securityGroupIds) && !vpc.securityGroupIds.some(isObject)) {
        params.VpcConfig.SecurityGroupIds = vpc.securityGroupIds;
      }

      if (Array.isArray(vpc.subnetIds) && !vpc.subnetIds.some(isObject)) {
      ...
      const didVpcChange = () => {
        ...
        return isDeepStrictEqual(remoteConfigToCompare, localConfigToCompare);
      };
    ...
    if (functionObj.image) {
      const imageConfig = {};
      const remoteImageConfig = remoteFunctionConfiguration.ImageConfigResponse?.ImageConfig;

      if (isObject(functionObj.image)) {
      ...
      if (
        !isDeepStrictEqual(imageConfig, remoteImageConfig === undefined ? {} : remoteImageConfig)
      ) {
        params.ImageConfig = imageConfig;
      }
    }

    const updatableParams = { ...params };
    delete updatableParams.FunctionName;
    if (!Object.keys(updatableParams).length) {
