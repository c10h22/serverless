...
const { hasOwn, safeShallowAssign } = require('../../../../utils/safe-object');
...
const appendIfMissing = (items, item) => {
  if (!items.some((existingItem) => isDeepStrictEqual(existingItem, item))) {
    items.push(item);
  }
};
...
class AwsCompileFunctions {
  ...
  async getS3Client(region) {
    if (!this.s3ClientPromisesByRegion) this.s3ClientPromisesByRegion = new Map();

    const cacheKey = JSON.stringify({
      region: region === undefined ? ['provider-default'] : ['explicit', region],
    });
    ...
    return this.s3ClientPromisesByRegion.get(cacheKey);
  }
  ...
  compileRole(newFunction, role) {
    ...
    if (typeof role === 'string') {
      if (role.startsWith('arn:')) {
        ...
        // role is the default role generated by osls
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.image) {
      ...
      if (isObject(functionObject.image)) {
        const imageConfig = {};
    ...
    if (kmsKeyArn) {
      if (typeof kmsKeyArn === 'string') {
        ...
        const iamRoleLambdaExecution = cfTemplate.Resources.IamRoleLambdaExecution;
        if (iamRoleLambdaExecution) {
          appendIfMissing(iamRoleLambdaExecution.Properties.Policies[0].PolicyDocument.Statement, {
            Effect: 'Allow',
            Action: ['kms:Decrypt'],
            Resource: [kmsKeyArn],
          });
    ...
    if (functionObject.environment || this.serverless.service.provider.environment) {
      ...
      functionResource.Properties.Environment.Variables = safeShallowAssign(
        {},
    ...
    if (
      ...
      const layerConfigurations = structuredClone(
      ...
      const functionProperties = structuredClone(functionResource.Properties);
    ...
    if (functionObject.durableConfig) {
      ...
      // Durable functions require their event source mappings to target a
      // qualified ARN (version/alias). durableConfig forces versioning above, so
      // publish a stable `current` alias on that version and record it as the
      // function's targetAlias so resolveLambdaTarget qualifies downstream ESMs.
      const aliasLogicalId = this.provider.naming.getLambdaDurableAliasLogicalId(functionName);
      const aliasName = this.provider.naming.getLambdaDurableEnabledAliasName();
      ...
      cfTemplate.Resources[aliasLogicalId] = {
        ...
        Properties: {
          FunctionName: { Ref: functionLogicalId },
          FunctionVersion: { 'Fn::GetAtt': [functionObject.versionLogicalId, 'Version'] },
          Name: aliasName,
        },
        DependsOn: functionLogicalId,
      };
  ...
  compileFunctionUrl(functionName) {
    ...
    if (url.cors) {
      ...
      if (url.cors.exposedResponseHeaders) {
        cors.exposedResponseHeaders = uniq(url.cors.exposedResponseHeaders);
      }
  ...
  ensureTargetExecutionPermission(destinationsProperty) {
    if (typeof destinationsProperty === 'string') {
      if (this.seenTargetExecutionPermissionKeys.has(destinationsProperty)) return;
      this.seenTargetExecutionPermissionKeys.add(destinationsProperty);
    } else if (isObject(destinationsProperty)) {
      if (this.seenTargetExecutionPermissionObjects.has(destinationsProperty)) return;
      this.seenTargetExecutionPermissionObjects.add(destinationsProperty);
    }
