...
const getWords = (value) => String(value).match(WORD_RE) || [];

const startCase = (value) => getWords(value).map(upperFirst).join(' ');

const camelCase = (value) => {
  const [firstWord = '', ...restWords] = getWords(value).map((word) => word.toLowerCase());
  return firstWord + restWords.map(upperFirst).join('');
};
...
module.exports = {
  ...
  getStackChangeSetName() {
    return `${this.getStackName()}-change-set`;
  },
  ...
  // Role
  getRolePath() {
    const customRolePath =
      this.provider.serverless.service.provider.iam &&
      this.provider.serverless.service.provider.iam.role &&
      this.provider.serverless.service.provider.iam.role.path;
  ...
  getRoleName() {
    const customRoleName =
      this.provider.serverless.service.provider.iam &&
      this.provider.serverless.service.provider.iam.role &&
      this.provider.serverless.service.provider.iam.role.name;
    return (
      customRoleName || {
        'Fn::Join': [
          '-',
  ...
  getApiGatewayName() {
    ...
    return this.provider.serverless.service.provider.apiGateway &&
      this.provider.serverless.service.provider.apiGateway.shouldStartNameWithService
  ...
  getModelLogicalId(schemaId) {
    return `ApiGateway${upperFirst(camelCase(schemaId))}Model`;
  },
  getEndpointModelLogicalId(resourceId, methodName, contentType) {
    return `${this.getMethodLogicalId(resourceId, methodName)}${startCase(contentType).replace(
      / /g,
      ''
    )}Model`;
  },
  ...
  getSchedulerScheduleLogicalId(functionName, scheduleIndex) {
    return `${this.getNormalizedFunctionName(functionName)}SchedulerSchedule${scheduleIndex}`;
  },
  ...
  getStreamConsumerName(functionName, streamName) {
    const consumerNamingMode =
      this.provider.serverless.service.provider.kinesis &&
      this.provider.serverless.service.provider.kinesis.consumerNamingMode;
    if (consumerNamingMode) {
  ...
  // ActiveMQ
  getActiveMQEventLogicalId(functionName, queueName) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
  ...
  getHttpApiName() {
    ...
    return this.provider.serverless.service.provider.httpApi &&
      this.provider.serverless.service.provider.httpApi.shouldStartNameWithService
      ? `${this.provider.serverless.service.service}-${this.provider.getStage()}`
      : `${this.provider.getStage()}-${this.provider.serverless.service.service}`;
  },
